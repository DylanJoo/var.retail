...
var.iter.est=function(X.big, Y.vec, k,
  ...
  # LOOP-step(4): Decompose the estimated inverse covariance matrix into PP'
  # LOOP-step(5): Transform the original X & Y into X' & Y'
  # LOOP-step(6): Estimate theta & select lambda1  @ iteration t+1
  # LOOP-step(7): Calculate the sample covariance matrix
...
var_meboot=function(Y.boot, X.boot, thetas, k){
  ...
  # subset of k bootrapped populations.
  ###################
  rss = c()
  df = c()
  kl = c()
...
var_BIC1=function(Y, X, thetas, k, Omega, scaling=T){
  ...
  for(i in 1:ncol(thetas)){
    ...
    if (is.null(Omega)){
      ll[i] = (-1/2)*(n*k*log(2*pi)) + (-1/2) * rss[i]
    }else{
