...
SKUAgg=function(IRI.table, topk=10){
  ...
  ############################################
  IRI.table$PZ.ADJ=(IRI.table$PRICE - IRI.table$FIXED.PRICE) / IRI.table$FIXED.PRICE
...
CateAgg=function(IRI.table){
  ...
  pz.adjusted = aggregate(IRI.table$PZ.REDUCT~IRI.table$WEEK, IRI.table, mean)
  price.fixed = aggregate(IRI.table$FIXED.PRICE~IRI.table$WEEK, IRI.table, mean)
  ...
  data = Reduce(function(x, y) merge(x=x, y=y, by=c('IRI.table$WEEK')), 
                     list(units, sales, 
                          price, price.weighted, price.fixed, pz.adjusted, 
                          promotion, advertisement, display))
  colnames(data) <- c('WEEK', 'UNITS', 'SALES', 
                      'PRICE', 'PRICE.W', 'PRICE.F', 'PZ.ADJ',  
                      'PR.RATE', 'AD.RATE', 'DI.RATE')
  data$PR.RATE = data$PR.RATE / nsku
  data$AD.RATE = data$AD.RATE / nsku
  data$DI.RATE = data$DI.RATE / nsku
  # PZ adjusted: (1) Change rate than aggreate (2) Aggreagate than change rate
  data$PZ.ADJ = (data$PRICE - data$PRICE.F) / data$PRICE.F
